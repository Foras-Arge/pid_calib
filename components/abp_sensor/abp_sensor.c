...
/**
 * @brief Initialize the Honeywell ABP sensor
 */
                          ...
                          gpio_num_t scl, gpio_num_t sda)
{
    ...
    i2c_master_bus_config_t cfg = {
        .clk_source = I2C_CLK_SRC_PLL_F80M,
        .i2c_port = port,
        .scl_io_num = scl,
        .sda_io_num = sda,
        .glitch_ignore_cnt = 7,
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "add dev failed: %s", esp_err_to_name(ret));
...
/**
 * @brief Apply optional configuration to the sensor
 */
...
esp_err_t abp_sensor_read(abp_sensor_t *sensor, int *value)
{
    ...
    uint8_t data[2];
    esp_err_t ret = i2c_master_receive(sensor->dev, data, sizeof(data), 1000 / portTICK_PERIOD_MS);
...
/**
 * @brief Calibrate zero offset by averaging samples
 */
...
/**
 * @brief Perform a communication check with the sensor
 */
...
/**
 * @brief Release I2C resources used by the sensor
 */
